...
**每条知识的内容控制在 200 字以内**（embedding 上限 250 token，超长自动截断）。
...
async def process_file_for_knowledge(
    ...
    try:
        ...
        for item in items:
            ...
            # 后端长度验证：content 超 250 token 则跳过
            if not _check_knowledge_length(content):
                skipped += 1
                continue
            ...
            vec_blob = _embed_knowledge_item(content)
...
def _check_knowledge_length(content: str) -> bool:
    """检查内容是否超 250 token。True=通过。"""
    try:
        from .embedding_service import get_embedding_service
        svc = get_embedding_service()
        if svc.is_available:
            return svc._count_tokens(content) <= 250
    except Exception:
        pass
    return len(content) <= 250  # 降级：按字符数估算


def _embed_knowledge_item(content: str) -> Optional[bytes]:
